        ...
        <el-tabs v-model="activeName">
              ...
              <el-form ref="timeForm" :model="timeForm" :rules="rules"  label-width="120px">
                <div class="form-cent">
                  ...
                  <el-form-item label="填写时间：">
                      ...
                      <el-radio v-model="timeForm.type" label="1">每个月规定日期填写</el-radio>
                  </el-form-item>
                </div>
                <template v-if="timeForm.type=='1'">
                  <div class="form-cent">
                    <h2 class="tab-sub-title">填报规定时间设置</h2>
                    <p class="tab-cont">请选择年份并填写每月份的填报开始日期到截止日期。举例：选择了1月1日-1月10日，系统将在1月1日至1月10日允许查看和编辑。1月10日至1月31日只允许查看，不允许编辑</p>
                    ...
                    <el-row v-if="currentYear" style="width: 80%">
                      <el-col
                        v-for="(monthItem,index) in currentYear.monthDataList"
                        :key="index"
                        :span="12">
          ...
          <el-tab-pane label="全局权限人员" name="second">
            <div class="form-wrap">
              ...
              <el-button type="primary" @click="savePermissionList">保存</el-button>
            </div>
    ...
    methods: {
      changeSite(){
        ...
        if(this.activeName==='first'){
            ...
            if(res.data.fillInTimes){
              res.data.fillInTimes.map(item=>{
                item.monthDataList = item.monthDataList.map(item2=>{
                  item2.fillMonth = parseInt(item2.fillMonth)
                  let newItem2 = {
                ...
                item.monthDataList = item.monthDataList.sort(function (a, b) {
                  return a.fillMonth - b.fillMonth
                })
              });
            }
            this.timeForm ={
          ...
          //第二个页签 全局权限人员设置
          vo.permissionType = 1;
          this.$api.pim.config.getUserPermissionList(vo).then(res => {
            if (res.code === 0) {
      ...
      savePermissionList(){
        ...
        this.$api.pim.config.savePermissionList(vo).then(res => {
          if (res.code === 0) {
            this.$message({
              type: "success",
              message: "保存成功！"
