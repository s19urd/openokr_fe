    ...
    <d2-container>
      ...
      >
        ...
        <template slot="tableColumns">
          <el-table-column fixed="left" label="操作" width="350px">
            <template slot-scope="props">
              ...
              <template  v-if="props.row.isBatchSync === '1'">
                <el-button type="text" size="mini" @click="degradeDevice(props.row)">设备关系下发</el-button>
                <el-button  type="text" size="mini" @click="degradeUser(props.row)">用户关系下发</el-button>
              </template>
            </template>
          </el-table-column>
          <el-table-column
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //设备关系下发
      degradeDevice(item){
        this.$api.energy.ecaSystemSiteRel.degradeDevice(item.id).then(res => {
          if(res.code==0){
            let message = '下发成功';
            this.$message({
              message,
      ...
      //用户关系下发
      degradeUser(item){
        this.$api.energy.ecaSystemSiteRel.degradeUser(item.siteId,item.outSysId,item.parkSystemName).then(res => {
          if(res.code==0){
            let message = '下发成功';
            this.$message({
              message,
